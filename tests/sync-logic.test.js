...
test("buildCloudPayload 附加 updatedAt 並保留原始欄位", () => {
  const state = { streak: { count: 3, lastDate: "2026-01-01" }, units: {}, wrongBook: [], rewards: normalizeRewards() };
...
test("mergeState：雲端無資料（首次登入）時直接沿用本機資料", () => {
  const local = {
    streak: { count: 5, lastDate: "2026-02-01" },
    units: { "frac-mul": { attempts: 10, correct: 8, bestAccuracy: 80, lastDate: "2026-02-01", completedQuestionIds: ["q1", "q2"] } },
    wrongBook: [],
    rewards: normalizeRewards({ xp: 100 }),
  };
  const merged = mergeState(local, null, "2026-02-01");
  assert.equal(merged.streak.count, 5);
  assert.equal(merged.units["frac-mul"].attempts, 10);
  assert.equal(merged.rewards.xp, 100);
  assert.ok(merged.updatedAt);
});

test("mergeState：本機無資料（新裝置）時直接沿用雲端資料", () => {
  const cloud = {
    streak: { count: 3, lastDate: "2026-02-01" },
    units: { "frac-div": { attempts: 6, correct: 5, bestAccuracy: 83, lastDate: "2026-02-01", completedQuestionIds: ["q9"] } },
    wrongBook: [{ unitId: "frac-div", questionId: "q9", date: "2026-02-01" }],
    rewards: normalizeRewards({ xp: 40 }),
  };
  const merged = mergeState(null, cloud, "2026-02-01");
  assert.equal(merged.streak.count, 3);
  assert.equal(merged.units["frac-div"].attempts, 6);
  assert.equal(merged.wrongBook.length, 1);
...
test("mergeState：兩邊都有資料時，單元進度採聯集/取較大值，不遺失任何一邊", () => {
  const local = {
    ...
    wrongBook: [{ unitId: "frac-mul", questionId: "q4", date: "2026-02-01" }],
  ...
  const cloud = {
    ...
    wrongBook: [{ unitId: "frac-mul", questionId: "q4", date: "2026-01-30" }],
  ...
  const unit = merged.units["frac-mul"];
  ...
  assert.equal(merged.wrongBook.length, 1);
  assert.equal(merged.wrongBook[0].date, "2026-02-01");
...
test("mergeState：不會因為合併而遺失任一邊獨有的單元", () => {
  const local = {
    streak: { count: 1, lastDate: "2026-03-01" },
    units: { "frac-mul": { attempts: 3, correct: 2, bestAccuracy: 66, lastDate: "2026-03-01", completedQuestionIds: ["a"] } },
    wrongBook: [],
    rewards: normalizeRewards(),
  };
  const cloud = {
    streak: { count: 1, lastDate: "2026-03-01" },
    units: { "frac-div": { attempts: 4, correct: 4, bestAccuracy: 100, lastDate: "2026-03-01", completedQuestionIds: ["b"] } },
    wrongBook: [],
    rewards: normalizeRewards(),
  };
  const merged = mergeState(local, cloud, "2026-03-01");
  assert.ok(merged.units["frac-mul"]);
  assert.ok(merged.units["frac-div"]);
});
