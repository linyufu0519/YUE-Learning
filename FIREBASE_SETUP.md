...
康軒六上 79 關使用 `semesterProgress` 欄位同步：

- `completedActions`：每關已完成的 `lesson`、`practice`、`mastery` 集合。
- `claimedUnitRewards`：已取得的單元獎勵 ID 集合。
- `stageStats`：各關作答次數、答對數、完成題號與近期題號。
- 六上 XP 不在裝置間相加，而是由合併後的完成集合與單元獎勵重新計算，上限 9900 XP。
- 合併採集合聯集與統計最大值，避免離線多裝置完成後遺失紀錄或重複發獎。
- 原有 `rewards.xp` 仍保留為歷史／一般 XP，不會在 migration 時被清空或轉換。
